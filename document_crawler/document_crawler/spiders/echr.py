...
class EchrSpider(scrapy.Spider):
    ...
    base_main_url = ('http://hudoc.echr.coe.int/app/query/results?query='
        ...
        '%20AND%20((documentcollectionid%3D%22GRANDCHAMBER%22)%20OR%20'
        '(documentcollectionid%3D%22CHAMBER%22)))%20XRANK(cb%3D14)%20'
        'doctypebranch%3AGRANDCHAMBER)%20XRANK(cb%3D13)%20doctypebranch'
        ...
        'ADVISORYOPINIONS)%20XRANK(cb%3D4)%20doctypebranch%3AREPORTS)%20'
        'XRANK(cb%3D3)%20doctypebranch%3AEXECUTION)%20XRANK(cb%3D2)%20'
        'doctypebranch%3AMERITS)%20XRANK(cb%3D1)%20doctypebranch%3A'
        'SCREENINGPANEL)%20XRANK(cb%3D4)%20importance%3A1)%20XRANK(cb%3D3)%20'
        'importance%3A2)%20XRANK(cb%3D2)%20importance%3A3)%20XRANK(cb%3D1)%20'
        'importance%3A4)%20XRANK(cb%3D2)%20languageisocode%3AENG)%20XRANK(cb%3'
        'D1)%20languageisocode%3AFRE&select=sharepointid,Rank,itemid,docname,'
        'doctype,application,appno,conclusion,importance,originatingbody,'
        'typedescription,kpdate,kpdateAsText,documentcollectionid,'
        'documentcollectionid2,languageisocode,extractedappno,isplaceholder,'
        'doctypebranch,respondent,respondentOrderEng,ecli&sort=&'
        'start={start_index}&length={length}&'
